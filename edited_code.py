...
def process_detection_data(detection_frames):
   ...
   for frame_id, detections in detection_frames:
       count = {"frame_id": frame_id, "car": 0, "pedestrian": 0, "bycicle": 0, "bus": 0, "person": 0,"traffic light":0,"truck":0}
...
# 打开视频文件
cap = cv2.VideoCapture("testvideo.mp4")  # 替换为你的视频路径
frame_id = 1
detection_frames = []
while cap.isOpened():
   ret, frame = cap.read()
...
if detection_results:
   ...
   # 使用 Plotly 生成折线图
   fig = px.line(df, x="frame_id", y=["car", "pedestrian", "bycicle", "bus", "person","traffic light","truck"],
